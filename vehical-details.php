...
<script>
    ...
    // Set the minimum allowed date for "From Date" to today
    fromDateInput.setAttribute('min', today.toISOString().split('T')[0]);

    // Update "To Date" constraints when "From Date" changes
    fromDateInput.addEventListener('change', function() {
        var fromDate = new Date(this.value);
        var maxToDate = new Date(fromDate);

        maxToDate.setDate(fromDate.getDate() + 31); // Maximum 31 days from "From Date"

        // Set the min and max attributes for "To Date"
    ...
    toDateInput.addEventListener('change', function() {
        ...
        maxDate.setDate(fromDate.getDate() + 31); // 31 days from "From Date"

        if (toDate < fromDate) {
            alert("To Date must be after From Date.");
            this.value = ''; // Clear invalid input
        } else if (toDate > maxDate) {
            alert("To Date must be within 31 days of From Date.");
            this.value = ''; // Clear invalid input
        }
    });
});
